...
Single Layer Perceptron (SLP) - Klasifikasi biner Iris (Setosa vs Versicolor)

Satu file berisi semuanya: load data, model, training loop, dan simpan hasil.
Mereplikasi persis logika di spreadsheet SLP (forward pass, sigmoid,
backpropagation, update bobot per sampel) sehingga hasil akurasi & loss-nya
sama dengan hasil di gsheet.

Jalankan:
    python src/train.py
Output (folder outputs/):
    results.csv          -> rekap loss & akurasi per epoch
    loss_chart.png        -> loss training vs validasi, 1 grafik gabungan
    accuracy_chart.png     -> akurasi training vs validasi, 1 grafik gabungan
"""
...
# =====================================================================
# DATA: baca Excel + split train/validation (programatik, bukan manual)
# =====================================================================
...
def split_train_validation(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Split train/validation secara programatik pakai filter + slice pandas.

        Training   (80 baris/epoch, sama persis diulang tiap epoch):
            40 Setosa pertama (Data baris 1-40) + 40 Versicolor pertama (baris 51-90)
        Validation (20 baris/epoch, sama persis diulang tiap epoch, held-out):
            10 Setosa berikutnya (Data baris 41-50) + 10 Versicolor berikutnya (baris 91-100)
    """
...
def train_and_validate(train_df: pd.DataFrame, val_df: pd.DataFrame,
    ...
    Loop 5 epoch:
      ...
      - Validasi: bobot "dicabang" dari bobot training di akhir epoch itu
        (setelah update dari sampel training TERAKHIR epoch tsb juga ikut
        masuk), ikut ter-update sepanjang sampel validasi, tapi TIDAK
        memengaruhi bobot training -- tiap epoch validasi selalu mulai
        dari cabang baru.
    """
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _, val_loss, val_acc = run_epoch(weights, val_df, lr)  # dicabang, tidak menimpa `weights`
...
def save_metric_chart(results: list[dict], train_key: str, val_key: str,
    ...
    """Satu fungsi dipakai ulang buat chart loss MAUPUN akurasi -- gabung
    training & validasi dalam 1 grafik (bukan 2 grafik terpisah)."""
    ...
    plt.ylim(bottom=0)  # sumbu Y mulai dari 0 -- biar perbandingan nggak menyesatkan
    if as_percent:
